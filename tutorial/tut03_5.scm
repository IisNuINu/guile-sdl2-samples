  ...
  (let ([angle 0]        [scale 1]        [delta 0.01])
    (lambda (cr w h)
      ...
      (let ([radius   40]
        ...
        (CAI:cairo-set-source-rgb      cr 0  0  1)
        (CAI:cairo-rectangle           cr 0  0  w h)
        (CAI:cairo-fill                cr)
...
(define (Render ren s-rect d-rect)
  ...
  (let* ([w (mySDL:rect-w s-rect)]
         ...
         [i       (mySDL:make-rgb-surface w h 32)])
    ;;(SDL:convert-surface-format i 'bgra8888)
    ;;(SDL:convert-surface-format i 'rgb24)
    ;;(SDL:convert-surface-format i 'rgb24)
    (let* ([surface (CAI:cairo-image-surface-create-for-data (SDL:surface-pixels i)
      ...
      ;;(display "sdl2  surf: ") (display (SDL:surface-pixel-format          i))        (newline)
      ;;(display "cairo surf: ") (display (CAI:cairo-image-surface-get-format surface)) (newline)
;;sdl2  surf: #<pixel-format name: argb8888 bpp: 4>
;;cairo surf: argb32
      (do-draw cr w h)
